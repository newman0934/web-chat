...
- `app/ws/manager.py`:`connect` 回傳 `is_first`(該 user 由 0→1 條);`disconnect` 回傳 `is_last`(由 1→0 條);另以記憶體保存 last_seen:`mark_last_seen(user_id, dt)` / `get_last_seen(user_id)`。
- `app/services/presence.py`:
  ...
  - **(不寫 DB)** —— last_seen 改存 manager 記憶體(見實作註記)。
- `app/ws/router.py`:connect 後若 `is_first` → 讀好友清單快取 + 廣播 online;disconnect 後若 `is_last` → `manager.mark_last_seen`(記憶體)+ 用快取好友清單廣播 offline。helper `_emit_presence(friend_ids, user_id, online, last_seen)` 只送 `manager.is_online` 的好友、不碰 DB。
  - **WHY 不在斷線寫 DB**:starlette TestClient 關閉路徑中於 disconnect 開 DB session 會死結;連線當下頻繁寫 DB 在整體測試下亦誘發 SQLite 死結。故 presence 全程不從 WS 生命週期寫 DB。
- `app/routers/contacts.py`:`list_contacts` 帶入 `online`(`manager.is_online`)與 `last_seen_at`(`manager.get_last_seen`)。
